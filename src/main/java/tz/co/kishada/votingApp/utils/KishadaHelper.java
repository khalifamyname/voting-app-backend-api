...
@Slf4j
public class KishadaHelper {
    private static final Logger logger = LoggerFactory.getLogger("TAUSI-GENERAL-LOG");
    ...
    public String httpTermisDebtRequest(final String plateNumber,String termUrl) throws IOException {
        try {
            String fullUrl = termUrl+plateNumber;
            final CloseableHttpClient httpClient = HttpClients.createDefault();
            final HttpGet getRequest = new HttpGet(fullUrl);
           // getRequest.addHeader("Content-Type", "application/json; charset=UTF-8");
            getRequest.addHeader("X-Transfer-Key", "e9f3e572-db87-4eff-9ed6-66922f1f7f24");
            logger.info("Data we send to TERMIS : {}",plateNumber);
            logger.info("Data we send to URL  : {}",termUrl);
            final CloseableHttpResponse response = httpClient.execute(getRequest);
            final String responseString = EntityUtils.toString(response.getEntity());
            response.close();
            logger.info("Response from TERMIS  is : {}",responseString);
    ...
    public String httpTraRequest(final String data, final String requestURL) throws IOException {
        try {
            // VehicleScannerResponse newVehicleScannerResponse = new VehicleScannerResponse();
            final CloseableHttpClient httpClient = HttpClients.createDefault();
    ...
    /**
     * method to convert file to base 64 string
     * @param filePath
     * @return
     */
    public static String convertFIleToBase64(String filePath){
        ...
        try {
            ...
            log.error("Failed to find a file : "+ e);
            e.printStackTrace();
        }
